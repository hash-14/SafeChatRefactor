...
public class autoreply_add extends AppCompatActivity {
...
int count = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_autoreply_add);


        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        Toolbar mToolbar = (Toolbar)findViewById(R.id.main_page_toolbar);
        setSupportActionBar(mToolbar);
        getSupportActionBar().setTitle("Add auto reply message");
...
list.clear();
        ...
        mAutoReplyDatabaseFromMe = FirebaseDatabase.getInstance().getReference().child("Auto_reply_From_Me");
        ...
        BSch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {
                    Toast.makeText(autoreply_add.this,"Start date cannot be greater than the end date", Toast.LENGTH_LONG).show();
                    return;
                }
                else
                {
                ...
                if (time_from_h > time_to_h) {
                        Toast.makeText(autoreply_add.this, "Start time cannot be greater than the end time", Toast.LENGTH_LONG).show();
                        return;
                    } else {
                        if (date_from_m >= date_to_y) {
                            Toast.makeText(autoreply_add.this, "Start time cannot be greater than the end time", Toast.LENGTH_LONG).show();
                            return;
                        }
                  }
                ...
                {
                    Toast.makeText(autoreply_add.this, "Empty message cannot be scheduled!", Toast.LENGTH_LONG).show();
                    return;
                }
                if(list.isEmpty())
                {
                    Toast.makeText(autoreply_add.this, "Select at least one friend!", Toast.LENGTH_LONG).show();
                    return;
                }


                SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(autoreply_add.this);
                /////////////save////////////////
            //    SharedPreferences.Editor editor = settings.edit();
            //    editor.putString("statepara1", ts);
             //   editor.commit();
                //////////////get//////////////////
                String ret = settings.getString("state_autoreply_list1", "");  /// 0 is default if variable not found

                int flag2=0;
                for(int i =0; i<list.size();i++) {
                   if(ret.contains( list.get(i).toString())) {
                       flag2=1;
                   }
                }
                if(flag2==1)
                {
                    Toast.makeText(autoreply_add.this, "Some friends have already previous auto reply message scheduled to them. Delete the previous message then try again.", Toast.LENGTH_LONG).show();
                    return;
                }
                ...
                SharedPreferences settings11 = PreferenceManager.getDefaultSharedPreferences(autoreply_add.this);
                SharedPreferences.Editor editor11 = settings11.edit();
                ...
                for(int i =0; i<list.size();i++) {
                    ...
                    mAutoReplyDatabase.child(user_id).child(uusser_id).setValue(object2).addOnSuccessListener(new OnSuccessListener<Void>() {
                        ...
                        public void onSuccess(Void aVoid) {
                            ...
                            if(countt==list.size())
                            {
                                Toast.makeText(autoreply_add.this, "Scheduling message done!", Toast.LENGTH_LONG).show();

                                Intent startIntent = new Intent(autoreply_add.this, autoreply.class);
        ...
        Bdatefrom.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                final Dialog dialog = new Dialog(autoreply_add.this);
    ...
    {
        count=0;
        firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<Friends, autoreply_add.UsersViewHolder>(
                Friends.class,
                R.layout.users_single_layout_autoreply_add,
                autoreply_add.UsersViewHolder.class,
                mfriendsCurrentUserDatabase

        ) {
            @Override
            protected void populateViewHolder(final autoreply_add.UsersViewHolder viewHolder, final Friends model, final int position) {
                count++;
           ...
           CB.setText(userID+";;;;;;;;;;"+name);
            ...
            CB.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if(c.isChecked())
                    {
                        autoreply_add.list.add(useridd);
                        autoreply_add.list2.add(namee);
                    ...
                    {
                        autoreply_add.list2.remove(useridd);
                        autoreply_add.list.remove(namee);
                    }
            ...
            if(x.equals("Yes")) {
                CB.setChecked(true);
                autoreply_add.list.add(userID);
                autoreply_add.list2.add(name);
            }
            else {
                CB.setChecked(false);
            }
